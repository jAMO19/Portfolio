...
const rasterData = [ 
    { file:'tree_mortality_2018_clipped.tif', label: '2018 Mortality' }, 
    { file:'tree_mortality_2019_clipped.tif', label: '2019 Mortality' },
    { file:'tree_mortality_2020_clipped.tif', label: '2020 Mortality' }
]; 
...
function initMap() {
    ...
    map = L.map('distribution-map-container', { 
        minZoom: 7,  // Minimum zoom level (e.g., to keep the whole region visible)
        maxZoom: 5000, // Maximum zoom level (e.g., to prevent raster pixelation)
        zoomSnap: 1, // Optional: ensures zoom levels stay at integers
        zoomDelta: 1 // Optional: distance jumped per zoom click
    }).setView([54.5, -126.5], 8); 
